...
export const createServiceAppointment = async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn("Duplicate check failed:", chkErr);
    }
    ...
    // Online booking (Stripe / PayPal / Mock Fallback)
    const frontendBase = buildFrontendBase(req);

    // 1. Try PayPal Checkout
    if (process.env.PAYPAL_CLIENT_ID) {
      // PayPal appends ?token=ORDER_ID&PayerID=PAYER_ID to return_url automatically
      const paypalSuccessUrl = `${frontendBase}/service-appointment/success`;
      const paypalCancelUrl = `${frontendBase}/service-appointment/cancel`;
      try {
        const paypalOrder = await createPayPalOrder(numericAmount, paypalSuccessUrl, paypalCancelUrl);
        if (paypalOrder && paypalOrder.orderId && paypalOrder.checkoutUrl) {
          const created = await ServiceAppointment.create({
            ...base,
            status: "Pending",
            payment: { method: "Online", status: "Pending", amount: numericAmount, sessionId: paypalOrder.orderId },
          });
          return res.status(201).json({ success: true, appointment: created, checkoutUrl: paypalOrder.checkoutUrl });
        }
      } catch (err) {
        console.error("PayPal service order creation error:", err);
      }
    }
    ...
    // 3. Mock checkout fallback
    const mockSessionId = `mock_session_service_${Date.now()}_${Math.random().toString(36).substring(7)}`;
    const created = await ServiceAppointment.create({
      ...base,
      status: "Pending",
      payment: { method: "Online", status: "Pending", amount: numericAmount, sessionId: mockSessionId },
    });
    const mockCheckoutUrl = `${frontendBase}/service-appointment/success?session_id=${mockSessionId}`;
...
export const confirmServicePayment = async (req, res) => {
  try {
    ...
    if (session_id.startsWith("mock_session_service_")) {
    ...
    // 2. PayPal Payment Check
      ...
      try {
        ...
        console.error("PayPal capture service order error:", err);
      }
    }
    ...
    if (!appt && stripe) {
      try {
        ...
        console.error("Stripe retrieve service session error:", err);
      }
    }
...
export const updateServiceAppointment = async (req, res) => {
  try {
    ...
    return res.json({ success: true, appointment: updated, data: updated });
  } catch (err) {
...
// Get Service Appointments by User (Clerk ID)
export const getMyServiceAppointments = async (req, res) => {
  try {
    const clerkUserId = resolveClerkUserId(req);
    const appointments = await ServiceAppointment.find({ createdBy: clerkUserId }).sort({ date: -1 });
    return res.json({ success: true, appointments, data: appointments });
  } catch (err) {
    console.error("getMyServiceAppointments error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
...
export const cancelServiceAppointment = async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: "Canceled successfully", appointment: appt });
  } catch (err) {
...
// Get Service Appointments Stats (Admin / Services View)
